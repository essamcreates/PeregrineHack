          ...
          {!sent && (
            <>
              <div class="h-full flex justify-end">
                <textarea
                  class="border-2 border-blue-800 bg-green-200 w-3/4 h-20 mt-3 p-1 rounded-lg"
              ...
              <div class="h-full flex justify-end">
                <button
                  class="w-1/5 h-full mt-1 border-2 border-blue-900 bg-teal-200 p-1 text-center rounded-md transition-transform transform hover:bg-teal-300"
                  onClick={() => {
      ...
      if (index === currentLevel.options.length - 1 && index % 2 === 0) {
        choices.push(
          <div class="col-span-2 flex items-center justify-center m-1" key={index}>
            <button
              class="w-11/12 h-full mt-3 m-2 border-2 border-blue-900 bg-red-200 p-1 text-center rounded-md transition-transform transform hover:bg-red-300"
              onClick={() => {
        ...
        choices.push(
          <div class=" flex items-center justify-center m-1" key={index}>
            <button
              class="w-10/12 h-full m-2 mt-2 border-2 border-blue-900 bg-red-200 p-1 text-center rounded-md transition-transform transform hover:bg-red-300"
              onClick={() => {
                handleOptionClick(option.displayText, option.sendText, option.next);
              }}
            >
    ...
    return (
      <div class="place-items-start">
        ...
        {currentLevel.message && (
          <div>
            <p class="border-2 border-blue-900 bg-blue-200 w-3/4 p-2 text-center rounded-md m-1">
              {currentLevel.message}
            </p>
          </div>
        )}
    ...
    const prev = previousSteps.map((stepId, index) => {
      ...
      prevText.options.map((option, index) => {
        if (index === prevText.options.length - 1 && index % 2 === 0) {
          choices.push(
            <div class="col-span-2 flex items-center justify-center m-1" key={index}>
              <button
                class="w-11/12 h-full mt-3 m-2 border-2 border-blue-900 bg-red-100 p-1 text-center rounded-md"
                disabled
              >
                {option.displayText}
              </button>
          ...
          choices.push(
            <div class=" flex items-center justify-center m-1" key={index}>
              <button
                class="ml-1 h-full w-10/12 border-2 border-blue-900 bg-red-100 p-1 text-center rounded-md"
                disabled
              >
                {option.displayText}
              </button>
      ...
      return (
        <div>
          ...
          {prevText.message && (
            <div>
              <p class="ml-1 border-2 border-blue-900 bg-blue-100 w-3/4 p-2 text-center rounded-md ">
                {prevText.message}
              </p>
            </div>
          )}
          ...
          <div class="flex justify-end">
            <p class="mr-1 w-2/5 border-2 border-blue-900 bg-green-100 p-1 text-center rounded-md">
              {prevChoices[index]}
            </p>
          </div>
        </div>
      ...
      {!usingChatBot && (
        <div>
          <div class=" flex items-center justify-center">
            <button
              class="bg-slate-400 ring-stone-400 ring-offset-1 ring-offset-stone-600 ring-4 p-20 rounded-lg"
          ...
          <div class=" flex items-center justify-center mt-20 ">
            <img
              onClick={() => setUsingChatBot(true)}
              src="src/assets/bird-removebg-preview.png"
              class="w-1/2 "
              alt="Image of Farai the bird"
            />
          </div>
        </div>
      )}
